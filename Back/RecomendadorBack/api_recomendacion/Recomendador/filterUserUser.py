...
class RecommendationUserUser:
  ...
  def predecirJuegosPorUsuario(self):
    ...
    print(prediccionVideoJuego[self.user_id].sort_values(ascending=False).to_dict())
    return (prediccionVideoJuego[self.user_id].sort_values(ascending=False)[:24].to_dict())
